...
import shutil
import numpy as np
from ..utils.draw_confusion_matrix import DrawConfusionMatrix
import cv2
...
class BaseRunner(object):
    ...
    def _inference(self):
        ...
        for _ in range(test_loader.get_epoch_size()):
            ...
            #
            #     dst_dir = '../dataset/error_batch/'
            #     if not os.path.exists(dst_dir):
            #         os.makedirs(dst_dir)
            #     for i in range(b):
            #         filename = batch['filenames'][i]
            #         gt = int(batch['gt'][i].cpu().numpy())
            #         pred = dump_results[i]['prediction']
            #
            #         if pred!= batch['gt'][i]:
            #             dstpath = os.path.join(dst_dir, str(gt) + '_' + filename.split('/')[-1])
            #             img = ((batch['image'][i].cpu() * std) + mean) * 255
    ...
    def evaluate(self):
        ...
        if self.args['debug']:
##########
            print(len(all_device_results_list[0]))
            count = 0
            totual = 0
            dst_dir = '../dataset/error/'
            error_dict = {}
            confusion_dict = {}
            if not os.path.exists(dst_dir):
                os.makedirs(dst_dir)
            label_name = [str(i) for i in range(89)]
            drawconfusionmatrix = DrawConfusionMatrix(labels_name=label_name)
            with open(dst_dir + 'pred.txt', 'w') as f:
                for batch in all_device_results_list[0]:
                    for res in batch:
                        totual+=1
                        drawconfusionmatrix.update(res['label'], res['prediction'])
                        if res['prediction'] == res['label']:
                            count += 1
                        else:
                            filename = res['filename']
                            filepath = os.path.join('../dataset',filename)
                            dstpath = filepath.replace('data/','error/')
                            shutil.copy(filepath,dstpath)
                            if str(res['label']) not in error_dict.keys():
                                error_dict[str(res['label'])] = 0
                            error_dict[str(res['label'])] += 1
                            # lst1 = list(map(lambda x: str(x), res['topk_idx']))
                            # str1 = ''.join(lst1)
                            f.write(str(res['label'])+' '+str(res['prediction'])+' '+res['filename'])
                            f.write('\n')
            ...
            for i in range(89):
                ...
                confuse = []
                for j in range(89):
                    if confusion_mat[i][j]>=0.005:
                        confuse.append([j,confusion_mat[i][j]])
                print(confuse)
            print(count/totual)
            print(error_dict)
##########
    ...
    def forward_train(self, batch):
        ...
        if self.args['debug']:
            ...
            dst_dir = '../dataset/vis/aug'
            if not os.path.exists(dst_dir):
                os.makedirs(dst_dir)
            dst_dir_ori = '../dataset/vis/ori'
            ...
            for i in range(b):
                ...
                gt = int(batch['gt'][i].cpu().numpy())
                dstpath = os.path.join(dst_dir,str(gt)+'_'+filename.split('/')[-1])
                dstpath_ori = os.path.join(dst_dir_ori,str(gt)+'_'+filename.split('/')[-1])
                img = ((batch['image'][i].cpu() * std ) + mean) *255
                img = img.permute(1,2,0).numpy()
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                cv2.imwrite(dstpath,img)
                filepath = os.path.join('../dataset', filename)
                shutil.copy(filepath,dstpath_ori)
            ######
    ...
    def forward_model(self, batch):
        ...
        # print('!' * 100)
        # flops, params = get_model_complexity_info(self.model, (3, 112, 112), as_strings=True, print_per_layer_stat=True)
        #
