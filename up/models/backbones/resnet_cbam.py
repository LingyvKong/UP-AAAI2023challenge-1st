...
__all__ = ['ResNet_cbam', 'resnet18_cbam']
...
class ResNet_cbam(nn.Module):
    ...
    def __init__(self,
        ...
        self.layer1 = self._make_layer(block, layer_in_planes[1], layers[0], stride=2,
                                       normalize=normalize,
                                       block_id=0)
...
def resnet18_cbam(pretrained=False, **kwargs):
    ...
    model = ResNet_cbam(Block, [1, 1, 1], **kwargs)
    return model
...
# 1 1 1 1 0.25效果 0.91
# 1 1 1   0.25效果 0.89
# 1 1 1   0.5，用3个3×3替代7×7，增加channel但是第一个layer也下采样，无残差结构， 速度居然很快
# 带res的onnx转的有问题，报错了 #没问题，是没加载进去 有残差性能差不多，但是慢一点
# 7x7快多了，计算量也小，0.21，但是低一点
# 回不去了？ 112就行了，回去了
# 后面都112